...
from inference_tool.k_diffusion import sampling as k_sampling

import inference_tool.schedulers as k_schedulers
...
class KDiffusionModelWrapper(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, sigma: torch.Tensor, **kwargs):
        ...
        if self.hook_fn is not None:
            self.hook_fn(x=x, sigma=sigma, denoised=denoised, cap_feats=self.cond["cap_feats"], cap_mask=self.cond["cap_mask"], neg_feats=self.uncond["cap_feats"], neg_mask=self.uncond["cap_mask"], v_pred=v_pred, v_pred_cond=v_pred_cond, v_pred_uncond=v_pred_uncond, current_timestep=current_timestep, guidance_scale=self.guidance_scale, shift=self.shift)
...
def sample_with_k_diffusion(
    ...
    **sampler_kwargs,
    ...
    wrapped_model = KDiffusionModelWrapper(
        ...
        neg_hidden_states, neg_attention_mask, hook_fn
    )
