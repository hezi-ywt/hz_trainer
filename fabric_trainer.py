...
class FabricTrainer:
    ...
    def fit(
        ...
        if ckpt_path is not None and os.path.isdir(ckpt_path):
            ...
            if latest_checkpoint_path is not None:
                self.load(self.state, latest_checkpoint_path)
        ...
        self.should_stop = False
    def on_train_batch_end(self, model: L.LightningModule, _current_train_return, batch, batch_idx, is_last=False):
        """
        ...
        self.save(model, self.state, is_last=is_last)
    ...
    def train_loop(
        ...
        for batch_idx, batch in enumerate(iterable):
            ...
            self.on_train_batch_end(model, self._current_train_return, batch, batch_idx, is_last)
    ...
    def training_step(self, model: L.LightningModule, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        fabric_module = getattr(model, "model", None)
        if hasattr(model, "get_module"):
            fabric_module = model.get_module()
        # 确保模型被正确设置
        ...
        with self.fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            if isinstance(outputs, Mapping):
                self.fabric.log_dict(outputs, step=self.global_step)
            else:
                self.fabric.log("train_loss", outputs, step=self.global_step)
    ...
    def save(self, model: L.LightningModule, state: Optional[Mapping], is_last: bool = False) -> None:
        ...
        if self.save_every_n_steps:
            is_ckpt_step = self.global_step % self.save_every_n_steps == 0 and self.global_step > 0
        else:
            is_ckpt_step = False
        if self.save_every_n_epochs:
            is_ckpt_epoch = self.current_epoch % self.save_every_n_epochs == 0 
        else:
            is_ckpt_epoch = False
        ...
        postfix = f"epoch_{self.current_epoch}_step_{self.global_step}"
        if hasattr(model, "save_checkpoint"):
            model.save_checkpoint(os.path.join(self.checkpoint_dir, f"checkpoint-{postfix}"),metadata)
        else:
            self.fabric.save(os.path.join(self.checkpoint_dir, f"checkpoint-{postfix}.ckpt"), state)
