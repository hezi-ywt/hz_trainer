...
class Lumina2ModelDMD(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        v_real_pred_cfg = v_real_negtive_pred + 5 * (v_real_pred - v_real_negtive_pred)
        # x1_pred = xt + v_pred * sigmas.view(-1, 1, 1, 1)
        # 计算损失
        terms = {}
        terms["task_loss"] = torch.mean(((v_pred - ut) ** 2), dim=list(range(1, len(x1.size()))))
        ...
        terms["loss"] = terms["task_loss"] +  terms["task_loss_cfg_distill"]
        terms["task_loss"] = terms["task_loss"].clone().detach()
        ...
        return {
            "loss": loss,
            "task_loss": terms["task_loss"],
            "task_loss_cfg_distill": terms["task_loss_cfg_distill"],
            "t": t
        }
...
if __name__ == "__main__":
    ...
    config = OmegaConf.create({
        "model": {
            ...
            "init_from": "/mnt/hz_trainer/checkpoints4/checkpoint-epoch_8_step_19000.safetensors",
            "use_ema": False,
