...
class Lumina2Model(pl.LightningModule):
    ...
    @torch.no_grad()
    def add_noise(self, x, sigma, generator=None):
        if generator is None:
            generator = torch.Generator(device="cpu").manual_seed(0)
        return x + sigma * torch.randn_like(x, generator=generator)
    ...
    def forward(self, batch):
        ...
        ids = torch.randint(0, len(sigmas), (batch_size,))
        latent = latents[ids]
        sigma = sigmas[ids]

        # 计算损失
        loss_dict = self.transport.training_losses(self.model, latents, model_kwargs)
        loss_1024 = loss_dict["loss"].sum() / batch_size
        loss = loss_1024 
        ...
        if random.random() < 0.1:
            width, height = images.shape[3], images.shape[2]
            factor = math.sqrt(1024*1024/width/height)
            if random.random() < 0.005:
                factor = math.sqrt(768*768/width/height)
            new_width= int(width*factor//16)*16
            new_height= int(height*factor//16)*16
            mode_list = ['nearest', 'bilinear', 'bicubic', 'area']
            mode = random.choice(mode_list)
            images = F.interpolate(images, size=(new_height, new_width),
...
config = {
    "model": {
        "model_name": "Lumina2",
        "vae_path": "/mnt/huggingface/Neta-Lumina-diffusers/vae",
        "text_encoder_path": "/mnt/huggingface/Neta-Lumina-diffusers/text_encoder",
